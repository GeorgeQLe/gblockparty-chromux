...
describe("five-approach shared interface system", () => {
  ...
  it("makes DETECT first-run and permanently available without controlling external terminals", async () => {
    ...
    expect(renderer).toContain('"Reserving…" : "Continue"');
    ...
    expect(renderer).toContain('"Reserving…" : "Start Fresh"');
    expect(renderer).toContain("acquireDetectionLease");
    expect(renderer).toContain("renewDetectionLease");
    expect(renderer).toContain("releaseDetectionLease");
    expect(renderer).toContain("30_000");
    expect(renderer).toContain("Reserving…");
    expect(renderer).toContain("Your settings are preserved");
