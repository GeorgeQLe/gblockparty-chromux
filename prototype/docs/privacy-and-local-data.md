...
| Vercel connection credentials | A Vercel CLI login or personal token is explicitly connected in the terminal-header setup wizard; future OAuth connections use the same store. CLI-login profiles contain no Vercel credential. | `~/.chromux/vercel-credentials.json` uses mode `0600`; non-secret CLI labels/account names are plaintext metadata, while token/OAuth profiles are inside one Electron `safeStorage`-encrypted payload. Plaintext tokens remain in the main process only and the token field is cleared after every attempt. | A removed token profile is deleted from the next atomic file replacement. OAuth removal also attempts remote revocation. Removing a CLI profile does not run `vercel logout` or modify Vercel-owned login files. | Validation, OAuth exchange/refresh/revocation, and later deployment operations contact Vercel. Token-backed CLI processes receive the credential through `VERCEL_TOKEN`, never argv or renderer payloads. |
