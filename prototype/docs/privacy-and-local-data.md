...
| macOS Dock attention badge | Threads attention changes after local filtering, grouping, Done, and Snooze rules. | The current nonnegative aggregate count is held in memory and passed to macOS for the Dock icon. It is not written to Chromux storage. | Replaced on each count change and cleared to zero when no visible attention remains or the app exits. | Only the number is passed to the local macOS application-badge API. Session names, prompts, paths, attention reasons, and content do not leave Chromux through this path. |
