...
| Agent hook files | Chromux starts and writes local hook helpers. | `~/.chromux/hooks-claude.json`, `~/.chromux/codex-notify.sh`, `~/.chromux/hooks-grok.json`, `~/.chromux/grok-hook.sh`, and `~/.grok/hooks/chromux-turn-signals.json` | Rewritten at startup. | Not sent by Chromux. Claude/Codex paths are passed at launch; Grok discovers `~/.grok/hooks/chromux-turn-signals.json` globally (the hook no-ops unless `CHROMUX_SESSION_ID` is set). |
