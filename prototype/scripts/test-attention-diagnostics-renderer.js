...
(async () => {
  ...
  'pending Codex input should show a spinner while remaining internally pending and unsafe for updates');
  ...
  expect(d.groupText().includes('TURNpending') && d.groupText().includes('TABworking'),
    'the next ordinary prompt should retain pending state with immediate spinner presentation');
