...
for (const theme of themes) {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), `chromux-${theme}-`));
  const e2ePath = path.join(tmpDir, 'capture-theme.js');
  const e2eOutPath = path.join(tmpDir, 'e2e.out');
  const screenshotPath = path.join(outputDir, `${theme}.png`);
  fs.writeFileSync(e2ePath, `
    new Promise((resolve) => {
      window.chromuxTestThemes.select(${JSON.stringify(theme)});
      document.querySelector('#btn-settings').click();
      setTimeout(() => resolve(JSON.stringify({ ok: true, theme: ${JSON.stringify(theme)} })), 350);
    })
  `);
  ...
  if (result.status !== 0 || !report.includes('"ok":true') || !fs.existsSync(screenshotPath)) {
    console.error('THEME_CAPTURE_FAIL', { theme, status: result.status, signal: result.signal, report, stderr: result.stderr });
    process.exit(1);
  }
  console.log(`THEME_CAPTURE_OK ${theme} ${screenshotPath}`);
}
